...
func (m *Msg) ackReply(ackType []byte, sync bool, opts ...PubOpt) error {
	...
	if isPullMode {
		...
		switch {
		case sync && ctx != nil:
			_, err = nc.RequestWithContext(ctx, m.Reply, payload)
		case sync:
			_, err = nc.Request(m.Reply, payload, wait)
		case bytes.Equal(ackType, AckAck):
			err = nc.PublishRequest(m.Reply, m.Sub.Subject, payload)
		case bytes.Equal(ackType, AckNak) || bytes.Equal(ackType, AckTerm):
